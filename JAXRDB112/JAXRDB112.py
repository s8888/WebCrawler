    ...
    for index, row in df.iterrows():
        try:
            ...
            d = {'ISS_DATE':'', 'TITL': title, 'ISS_CTNT': result['content'], 
                 'ISS_NO':'', 'RLT_RGL':'', 'FILES':','.join(FILES), 
...
def parsingTitle(soup, checkRange):
    try:
        ...
        df = pd.DataFrame(columns = ["WEB_ADDR", "CRL_DATE", "ISS_DATE", "TITL", "LNK_URL", "SOURCE_ORG"])
            ...
            try:
                ...
                for j in range(len(dates)):
                    ...
                    if date < strDate: # 若發文日期小於開始日期, 則結束爬取主旨
                        ...
                        break
                    idx[j] = True
                d = {"WEB_ADDR" : url, "CRL_DATE" : endDate, "ISS_DATE" : dates, 
                     "TITL" : titles, "LNK_URL" : links, "SOURCE_ORG" : sources}
        ...
        traceback.print_exc()
        return pd.DataFrame(columns = ["WEB_ADDR", "CRL_DATE", "ISS_DATE", "TITL", "LNK_URL", "SOURCE_ORG"])
